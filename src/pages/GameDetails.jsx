...
function GameDetails() {
  ...
  const addGamePlaying = async () => {
    ...
    try {
      const response = await fetch(
        `https://gamejournal-backend-2023.adaptable.app/games`,
        {
          method: "POST",
  ...
  const addGameNotPlaying = async () => {
    ...
    try {
      const response = await fetch(
        `https://gamejournal-backend-2023.adaptable.app/gamesNotPlaying`,
        {
          method: "POST",
          body: JSON.stringify(payload),
          headers: {
  ...
  return (
    <>
      ...
      <div className="container">
        ...
        <p className="score">
          <img className="metacritic" src={metacritic} />
          Metacritic score: {game.metacritic}
        </p>
        <p>Average playtime: {game.playtime} hrs</p>
        <button className="btnAdd" onClick={addGamePlaying}>
          Add to my games
        </button>
        <button className="btnNotInterested" onClick={addGameNotPlaying}>
          I'm not interested
        </button>
      </div>
    </>
